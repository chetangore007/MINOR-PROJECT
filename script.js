// script.js
// AgnoSutra prototype front-end logic
// Lightweight weighted-score classifier + rule engine + KB + charting
...
const KNOWLEDGE_BASE = {
  ...
  // This function is a stand-in for an ML model: it computes scores for each dosha
  // using weights inspired by your feature roles. It's explainable & replaceable.
  function modelPredict(features){
    // features: {heart_rate, sleep_hours, diet_type, stress, mood, water}
    // We'll compute base scores. Higher score => more likely.
    ...
    // Normalize roughly (using plausible min/max)
    ...
    // Pitta correlates with higher heart rate, high stress, lower sleep
    scores['Pitta'] += 0.5*n_hr + 0.6*n_stress + 0.3*(1-n_sleep);
    // Vata correlates with low sleep, irregular diet, high stress
    scores['Vata'] += 0.5*(1-n_sleep) + 0.4*n_stress + 0.3*(dietEnc===1 ? 1 : 0);
    // Kapha correlates with lower hr, more sleep, heavier diet
    scores['Kapha'] += 0.6*(1 - n_hr) + 0.5*n_sleep + 0.3*(dietEnc===0 ? 0.5 : 0);
  
    // Mood and water adjust:
    scores['Vata'] += 0.2*(1-n_mood); // low mood => Vata tilt
    scores['Kapha'] += 0.2*n_water;   // more water sometimes Kapha-friendly
    scores['Pitta'] += 0.1*(n_mood>0.6 ? 0.4 : 0); // good mood mild Pitta boost
  
    // Convert to "prediction"
    const basePred = Object.entries(scores).sort((a,b)=>b[1]-a[1])[0][0];
    return {scores, basePred};
  }
  
  // -------------------- Rule engine --------------------
  function ruleEngine(inputs, basePrediction){
    // inputs: same as features
    // We'll cast "votes" based on explicit Ayurvedic rules.
  ...
  function onAnalyze(){
    ...
    // Model prediction
    const {scores, basePred} = modelPredict(features);
    // Rule engine
    const {final, votes} = ruleEngine(features, basePred);
    // Recs
    const rec = generateRecommendations(final);
    // Render
    renderResults(basePred, final, votes, scores, rec, features);
    // Update chart
  ...
  function renderResults(basePred, finalPred, votes, scores, rec, features){
    ...
    document.getElementById('explanation').textContent = `Base model (weighted-score) suggested ${basePred}. Rule engine adjusted using sensors/questionnaire (HR: ${features.heart_rate} bpm, sleep: ${features.sleep_hours} h, stress: ${features.stress}).`;
    // recs
  ...
  function updateSensorMetrics(hr, temp, hum){
    document.getElementById('hrMetric').textContent = hr + " bpm";
    document.getElementById('tempMetric').textContent = temp + " °C";
    document.getElementById('humMetric').textContent = hum + " %";
  }
